...
{
	uint8_t *ptr = (uint8_t *)in;
	if (ptr[257] != 0x75 || ptr[258] != 0x73 || ptr[259] != 0x74 || ptr[260] != 0x61 || ptr[261] != 0x72)
	{
	...
	if (tarfs_debug)
		qemu_log("Found %d entries.", count);
	...
	while (memcmp(file->Signature, "ustar", 5) == 0)
	{
		...
		if (tarfs_debug)
			qemu_log("[%p] Name: %s; Size: %d", file, file->Name, filesize);
