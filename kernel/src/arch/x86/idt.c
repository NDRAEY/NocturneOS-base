...
void init_idt(void) {
    ...
    // 0x8F = Trap Gate => (Gate type = 1111 (32 bit trap gate) + DPL = 0 + Present bit)
    ...
    /* IRQs */
    // 0x8E = Interrupt Gate => (Gate type = 1110 (32 bit interrupt gate) + DPL = 0 + Present bit)
    ...
    /* System calls */
    // 0xEF = Trap Gate => (Gate type = 1111 (32 bit trap gate) + DPL = 11 (ring 3) + Present bit)
    // DPL 3 means that this gate is accessible from ring 3 (user mode).
