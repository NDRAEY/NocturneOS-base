...
use elf::{
    ElfBytes,
    abi::{
        R_X86_64_32, R_X86_64_PC32, SHT_PROGBITS, SHT_REL, STB_GLOBAL, STT_FUNC, STT_NOTYPE,
        STT_SECTION,
    },
    endian::AnyEndian,
...
impl<'data> ModuleLoader<'data> {
    ...
    fn load_elf(&'data mut self) -> Result<(), LoadError> {
        self.elf = match ElfBytes::<AnyEndian>::minimal_parse(&self.raw_data.as_ref().unwrap()) {
            Ok(elf) => Some(elf),
            Err(err) => {
    ...
    fn load_segments(&'data mut self) -> Result<(), LoadError> {
...
// impl ModuleLoader<'_> {
//     pub fn load_from_path(path: &str) -> Result<Self, LoadError> {
//         let mut module = Self::new().load_data(path)?;
